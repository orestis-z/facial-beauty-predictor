...
from facial_beauty_predictor.backbone.mtcnn.layer_factory import LayerFactory
from facial_beauty_predictor.backbone.mtcnn.network import Network
from facial_beauty_predictor.backbone.mtcnn.exceptions import InvalidImage
...
class PNet(Network):
    ...
    def _config(self):
        ...
        layer_factory.new_max_pool(
            name='pool1', kernel_size=(2, 2), stride_size=(2, 2))
...
class RNet(Network):
    ...
    def _config(self):
        ...
        layer_factory.new_prelu(name='prelu1')
        layer_factory.new_max_pool(
            name='pool1', kernel_size=(3, 3), stride_size=(2, 2))
        layer_factory.new_conv(name='conv2', kernel_size=(3, 3), channels_output=48, stride_size=(1, 1),
                               padding='VALID', relu=False)
        layer_factory.new_prelu(name='prelu2')
        layer_factory.new_max_pool(name='pool2', kernel_size=(
            3, 3), stride_size=(2, 2), padding='VALID')
        layer_factory.new_conv(name='conv3', kernel_size=(2, 2), channels_output=64, stride_size=(1, 1),
                               padding='VALID', relu=False)
        layer_factory.new_prelu(name='prelu3')
        layer_factory.new_fully_connected(
            name='fc1', output_count=128, relu=False)  # shouldn't the name be "fc1"?
        layer_factory.new_prelu(name='prelu4')
        # shouldn't the name be "fc2-1"?
        layer_factory.new_fully_connected(
            name='fc2-1', output_count=2, relu=False)
        layer_factory.new_softmax(name='prob1', axis=1)

        layer_factory.new_fully_connected(
            name='fc2-2', output_count=4, relu=False, input_layer_name='prelu4')
...
class ONet(Network):
    ...
    def _config(self):
        ...
        layer_factory.new_fully_connected(
            name='fc2-2', output_count=4, relu=False, input_layer_name='prelu5')

        layer_factory.new_fully_connected(
            name='fc2-3', output_count=10, relu=False, input_layer_name='prelu5')

    def _feed(self, image):
        return self._session.run(['onet/fc2-2/fc2-2:0', 'onet/fc2-3/fc2-3:0', 'onet/prob1:0', 'onet/fc1/fc1:0'],
...
class MTCNN(object):
    ...
    def __init__(self, weights_file: str = None, min_face_size: int = 20, steps_threshold: list = None,
        ...
        if weights_file is None:
            assert pkg_resources.resource_exists(
                __name__, "data/mtcnn_weights.npy")
        ...
        with self.__graph.as_default():
            self.__session = tf.compat.v1.Session(
                config=config, graph=self.__graph)
    ...
    def detect_faces(self, img) -> list:
        ...
        total_boxes, points, _ = self.detect_faces_raw(img)
    ...
    def __stage3(self, img, total_boxes, stage_status: StageStatus):
        ...
        num_boxes = total_boxes.shape[0]
        if num_boxes == 0:
            return total_boxes, np.empty(shape=(0,)), np.empty(shape=(0,))
